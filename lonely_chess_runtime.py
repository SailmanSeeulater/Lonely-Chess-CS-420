...
Silent runtime: no move-by-move output.
Only explicit print() calls produce output.
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python lonely_chess_runtime.py <game.pgn>", file=sys.stderr)
        sys.exit(1)
    try:
        LonelyChessRuntime().run(sys.argv[1])
